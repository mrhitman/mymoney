...
export class CategoriesResolver {
    ...
    async categories(@CurrentUser() user: User, @Args('type', { nullable: true }) type?: string): Promise<CommonCategoryDto[]> {
        return this.service.getAll(user, { type });
    }

    @UseGuards(GqlAuthGuard)
    @Query(returns => CommonCategoryDto)
    async category(@CurrentUser() user: User, @Args('id') id: string): Promise<CommonCategoryDto> {
